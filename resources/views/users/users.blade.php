...
<x-app-layout>
    <x-slot name="headerTitle">
        ...
        <x-bread-link :bread="true" text="المستخدمين"></x-bread-link>
    </x-slot>
    ...
    <div class="row">
        <div class="col-12">
                        ...
                        @foreach ($users as $user)
                            <tr>
                                ...
                                @elseif($user->roles[0]->name == 'teacher')
                                    <td>معلم</td>
                                @elseif($user->roles[0]->name == 'guest')
                                    <td>لم يتم تحديده</td>
                                @else
                                    <td>المدير</td>
                                @endif
                            ...
                            <div class="p-3 d-flex justify-content-between align-items-center">
                                <div class="d-flex">
                                    <form action="{{ route('users.approve') }}" method="POST">
                                        ...
                                        <button type="submit" class="btn btn-primary btn-sm">معلم</button>
                                    </form>
                                    <form action="{{ route('users.approve') }}" method="POST" class="mx-2">
                                        @csrf
                                        <input type="hidden" value="" name="userId" class="inputId">
                                        <input type="hidden" value="1" name="userRole">
                                        <button type="submit" class="btn btn-danger btn-sm">مشرف</button>
                                    </form>
                                </div>
                                <button type="button" class="btn btn-secondary btn-sm"
                                    data-dismiss="modal">الغاء</button>
            ...
            <x-custom-modal classes="createUserModal" headerVariant="primary">
                <x-slot name="title">اضافه مستخدم</x-slot>
                <x-slot name="body">
        ...
        <x-custom-modal classes="updateUserModal" headerVariant="success">
            <x-slot name="title"> تعديل المستخدم :
    ...
    <x-slot name="scripts">
        <script>
            $(function() {
                ...
                $(".btnCreateUserModal").click(e => {
                    e.preventDefault();
                    $(".createUserModal").modal("show");
                })

                $(".btnUpdateUserModal").click(e => {
                    e.preventDefault();
